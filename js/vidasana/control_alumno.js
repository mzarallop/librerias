...
function cargar_reporte()
{
	$.ajax({
		...
		data:"edad="+$("#edad").val()+
			...
			"&peso="+$("#peso").val()+
			"&imc="+$("#imc").val()+
			"&idcolegio="+$("#colegio").val()+
			"&indicador="+$("#indicador").val()+
			"&case=3",
		success:function(result){
			console.log("respuesta");
			$("#tablas").html(result);
			$("#form-nuevo").css({display:'inline'});
			$("#informacion").toggle("fast");
		}
	});
...
//calculos del estado nutricional
...
function remove_control(id){
		...
		$.ajax({
			...
			type:'post', 
			data:"id="+id+"&usuario="+$("#user").val()+"&case=4",
			success:function(result)
			{
				$("#tablas").html(result);
			}
